...
@Entity
public class DomusPalatinaFirstState extends PhaseState {

    public static final String phaseName = "DomusFirst";

    public Game game;
    public Player player;


    public DomusPalatinaFirstState(Game game, Player player){
        this.game = game;
        this.player = player;
        super.setName(phaseName);
    }
    public DomusPalatinaFirstState() {}
    ...
    public void next(Phase phase) {
        ...
        if(option == 2) {
            phase.setState(new DomusPalatinaSecondState(this.game, this.player));
        } else {
